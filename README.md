...
백엔드는 도메인 단위 패키지 구조를 따릅니다. 각 도메인 패키지는 보통 `controller / service / repository / entity / dto / converter` 로 구성됩니다.
...
프론트엔드는 `frontend/` 서브폴더에 독립된 Vite 프로젝트로 존재합니다.
...
**4. 세션 → 과제/제출/출석**
스터디 아래에 `POST /api/studies/{studyId}/sessions`로 회차(세션)를 만들고, 각 세션에 `POST .../assignments`로 과제를 등록합니다. 참여자는 `POST .../submissions`로 과제를 제출합니다. 세션이 끝나면 `PATCH .../attendances`로 참여자 출석 상태(`PRESENT`/`ABSENT`/`UNMARKED`)를 일괄 갱신합니다.
...
**6. 프론트엔드 연동**
프론트엔드는 위 API 전체를 실제로 호출하도록 연동되어 있으며(데모/목업 데이터 아님), 세션 쿠키 + CSRF 헤더 기반 인증을 `frontend/src/api/client`와 `AuthContext`에서 처리합니다.
...
즉 **재시작할 때마다 DB가 초기화되고 더미 데이터로 다시 채워지므로**, 테스트 중 직접 추가/수정한 데이터는 재시작 시 사라집니다. 서버는 `http://localhost:8080`에서 뜹니다.
...
개발 서버는 `http://localhost:5173`에서 뜨고, `/api` 요청은 `vite.config.ts`의 proxy 설정을 통해 `http://localhost:8080`으로 전달됩니다.

프론트엔드를 정적 빌드해서 백엔드가 직접 서빙하게 하려면:

```bash
cd frontend
npm run build   # 결과물이 src/main/resources/static/ 으로 빌드됨
```

이후 `./gradlew bootRun`으로 실행하면 `http://localhost:8080`에서 같은 origin으로 프론트/백엔드가 함께 서빙되어 별도 CORS 설정이 필요 없습니다.
...
> `leader1@test.com` / `password123!` 같은 계정은 예전 문서에 남아있지만 현재 `data.sql`에는 존재하지 않습니다. 로그인 테스트는 반드시 위 표의 계정을 사용하세요.
...
curl -b cookies.txt -c cookies.txt -X POST http://localhost:8080/api/auth/login \
  ...
  -d '{"email":"test1@gmail.com","password":"1234qwer"}'
...
| 과제 제출 | `GET/POST /api/studies/{studyId}/sessions/{sessionId}/assignments/{assignmentId}/submissions` | 제출 목록 조회/등록 |
| 과제 제출 | `PATCH/DELETE /api/studies/{studyId}/sessions/{sessionId}/assignments/{assignmentId}/submissions/{submissionId}` | 제출 수정/삭제 |
...
> `participant`(스터디 참여자) 도메인은 위 목록에 없습니다 — 아직 전용 API가 구현되지 않았습니다.
